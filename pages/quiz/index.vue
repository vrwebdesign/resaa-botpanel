      ...
      >
        ...
        <template #items="{item}">
          ...
          <td>
            <span
              v-if="item.send_time"
            >{{ item.send_time | persianDate('jYYYY/jMM/jDD HH:mm') | persianDigit }}</span>
            <span v-else>-</span>
          </td>
          <td class="text-xs-center">
            <v-icon
              :class="item.is_posted?'green--text':'red--text'"
            >{{item.is_posted?'la-check-circle':'la-times-circle'}}</v-icon>
          </td>
        </template>
      </vr-data-grid>
    </v-card>
...
export default class QuizPage extends Vue {
  ...
  headers = [
    ...
    { text: 'عنوان', align: 'right', value: 'title', width: '10%' },
    ...
    { text: 'وضعیت ارسال', align: 'center', value: 'is_posted', width: '120%' }
  ]
  filters = []
  get meta() {
